...
* @param[in] ec - errore code from getAllProperties.
 ...
 */
inline void
    handlePhysicalSecurityGetSubTree(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
        ...
        const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    ...
    if (subtree.size() > 1)
    {
        BMCWEB_LOG_ERROR("Found more than one Chassis Intrusion DBus object");
        messages::internalError(asyncResp->res);
        return;
    }
    ...
    if (serviceMap.empty())
    {
        BMCWEB_LOG_ERROR("Error getting service map of Chassis Intrusion object");
        messages::internalError(asyncResp->res);
        return;
    }
...
inline void
    handleDecoratorAssetProperties(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
...
inline void
    handleChassisGetSubTree(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                    const std::string& chassisId, const boost::system::error_code& ec,
                    const dbus::utility::MapperGetSubTreeResponse& subtree)
{
    if (ec)
                ...
                object : subtree)
    {
        ...
        sdbusplus::asio::getAllProperties(
            *crow::connections::systemBus, connectionName, path,
            ...
            [asyncResp, chassisId, path](const boost::system::error_code&,
                    const dbus::utility::DBusPropertiesMap& propertiesList) {
                     ...
                     const std::string& chassisId)
{
    ...
    getMainChassisId(asyncResp, [chassisId](const std::string& mainChassisId,
    const std::shared_ptr<bmcweb::AsyncResp>& aRsp)
    {
        if (chassisId == mainChassisId)
        {
            ...
            dbus::utility::getSubTree(
                ...
                std::bind_front(handlePhysicalSecurityGetSubTree, aRsp));
        }
    });
}
